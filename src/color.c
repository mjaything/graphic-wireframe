...
int get_light(int start, int end, double altitude_percentage)
{
	...
	result = (int)(((1 - altitude_percentage) * start) + (altitude_percentage * end));
	return (result);
}
...
int get_final_color(t_point current, t_point start, t_point end, t_point delta)
{
	...
	if (delta.x > delta.y)
		altitude_percentage = get_altitude_percentage(start.x, end.x, current.x);
	else
		altitude_percentage = get_altitude_percentage(start.y, end.y, current.y);
